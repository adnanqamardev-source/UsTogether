...
### Acceptance Criteria
- ✅ `app/page.tsx` uses `"use server"` or remains async (Server Component)
- ✅ Authentication logic moved to `<AuthWrapper />` (already exists)
- ✅ `Dashboard` component uses `next/dynamic` with `ssr: false` and loading fallback
...
### Acceptance Criteria
- ✅ New component `components/MemoryBoard.tsx` (or enhance existing if it exists)
...
### Acceptance Criteria
- ✅ Chat container uses dark background (`#1a1a2e`) with `backdrop-filter: blur()`
- ✅ Message bubbles: user messages right-aligned (purple), partner messages left-aligned (gray)
- ✅ Typing indicator shows "Partner is typing..." when partner is actively typing
- ✅ Read receipts show checkmark when message is read
- ✅ Emoji picker (via emoji library or browser native) integrated in input
- ✅ Message history persists in Firestore and loads on mount
- ✅ Soft drop shadows and 16–24px border radius for polish
- ✅ Mobile responsive (single column, full height)
- ✅ Auto-scroll to latest message on new message or load
...
### Acceptance Criteria
- ✅ New page at `/dashboard/stats` or modal accessible from Dashboard
- ✅ Display total couple points across all time
- ✅ Show current streak (daily/weekly) with visual counter
- ✅ List all unlocked achievements with unlock dates
- ✅ Chart quiz completion over last 30 days (bar or line chart)
- ✅ Activity heatmap showing which days the couple was active
- ✅ Comparison badge: e.g., "You're in the top 10% of active couples" (if leaderboard exists)
- ✅ Share stats button (copy to clipboard or social share)
...
| Feature | Impact | Effort | Risk | Recommended Order |
|---------|--------|--------|------|-------------------|
| Performance Optimization | Critical | Medium | Low | **1st** |
| Quiz Sync Debug | Critical | High | Medium | **2nd** |
| Memory Board | High | Medium | Low | **3rd** |
| Chat UI Refinement | Medium | Medium | Low | **4th** |
| Dashboard Analytics | Medium | Medium | Low | **5th** |
